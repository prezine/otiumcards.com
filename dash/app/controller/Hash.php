...
class Encryption
{
    ...
    public function hide_mail($email) {
        ...
        $mail_part[0] = str_repeat("*", strlen($mail_part[0]) - 3);
        return substr($email, 0, 3) . implode("@", $mail_part);
    }
    public function hide_phone($tel) {
        $len = strlen($tel);
        return substr($tel, 0, -($len - 4)).str_repeat('*', $len - 9).substr($tel, $len - 2, 3);
    }
